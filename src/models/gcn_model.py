...
class GCN(nn.Module):
    ...
    def forward(self, x):
        ...
        fs1 = nn.functional.interpolate(self.tconv1(gcfm1), size=gcfm2.size()[2:], mode='nearest') + gcfm2
        fs2 = nn.functional.interpolate(self.tconv2(fs1), size=gcfm3.size()[2:], mode='nearest') + gcfm3
        fs3 = nn.functional.interpolate(self.tconv3(fs2), size=gcfm4.size()[2:], mode='nearest') + gcfm4
        out1 = self.tconv4(fs3)
        out = self.tconv5(out1)
